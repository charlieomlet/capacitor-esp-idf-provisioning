...
public class EspProvisioningBLE {
    ...
    @PluginMethod
    public void searchESPDevices(
        String devicePrefix,
        ESPConstants.TransportType transport,
        ESPConstants.SecurityType security,
    ...
    public void connect(String deviceName, String proofOfPossession, ConnectListener listener) {
        ...
        Runnable connectionTimeoutTask = new Runnable() {
            ...
            public void run() {
                ...
                errorLog(
                    new Error(String.format("Timed out after %s seconds while trying to connect to device: %s", connectTimeout, deviceName))
                );
    ...
    private void debugLog(String message) {
        Log.d("capacitor-esp-provision", message);
    }

    private void errorLog(String message, Throwable error) {
        error.printStackTrace();
         Log.e("capacitor-esp-provision", message, error);
    }

    private void errorLog(Throwable error) {
        error.printStackTrace();
         Log.e("capacitor-esp-provision", error.getMessage(), error);
    }
}
